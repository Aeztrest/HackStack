...
export default function DashboardPage() {
  ...
  const [userProjects, setUserProjects] = useState<ProjectRequest[]>([])
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const [userProjectsRes, hostingDomainsRes, notificationsRes] = await Promise.all([
          fetch("/api/user-projects", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("/api/hosting-domains", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("/api/notifications"),
        ])

        if (!hostingDomainsRes.ok || !userProjectsRes.ok || !notificationsRes.ok) {
        ...
        const userProjectsData: ProjectRequest[] = await userProjectsRes.json()
        const notificationsData: Notification[] = await notificationsRes.json()

        setProjectRequests(userProjectsData.filter((p) => p.status === "Beklemede"))
        setHostingDomains(hostingDomainsData)
        setUserProjects(userProjectsData.filter((p) => p.status !== "Beklemede" && p.status !== "Tamamlandı"))
  ...
  const activeProjectsCount = userProjects.filter(
    (p) => p.status === "Başlandı" || p.status === "Devam Ediyor",
  ).length
  const pendingRequestsCount = projectRequests.filter((r) => r.status === "Beklemede").length
              ...
              {projectRequests.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                  {projectRequests.map((request) => (
                    <ProjectRequestCard key={request.project_id} request={request} />
                  ))}
                </div>
              )}
            </motion.section>
              ...
              {userProjects.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                  {userProjects.map((project) => (
                    <ProjectProgressCard key={project.project_id} project={project} />
                  ))}
                </div>
              )}
            </motion.section>
